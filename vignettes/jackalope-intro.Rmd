...
removes the Y chromosome (we'll assume we only sampled females), and 
...
Lastly, the $\theta$ parameter is the population-scaled mutation rate, which we
...
If we want to save the information for these haplotypes, we can output a VCF file
or FASTA files (one per haplotype):
...
This would result in the following files to be generated: 
...
per `evo_rates`.
Because "scrm assumes an infinite-sites model for both recombinations and mutations"
(`scrm` documentation), I'm converting the decimals used to represent each segregating
site's position to integers.
Many of the sites don't map onto a unique position on the small chromosomes in `ref`,
...
maximum tree depth of $10^7$ because we want branch lengths to be in generations:

```{r examples-phylogeny-tree}
tree <- rcoal(10)
tree$edge.length <- 1e7 * tree$edge.length / max(node.depth.edgelength(tree))
```
...
one_chrom <- function(.size) {
    ...
    trees <- sims$trees[[1]]
    # scrm output branch lenghts in units of 4*N0 generations, but we want just
    # generations:
