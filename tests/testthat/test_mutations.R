...
# Simulating mutations in R and gemino's C++ code and making sure produce
# the same output
    ...
    for (s in 0:(length(seqs)-1)) {
        ...
        while (m < n_muts && max_size > 0) {
            ...
            if (rnd < 0.5) {
                ...
                gemino:::add_substitution(vars, v = v, scaff = s,
                                          nucleo = str, new_pos_ = pos)
                substr(ts[(s+1)], pos+1, pos+1) <- str
            } else if (rnd < 0.75) {
                size = as.integer(rexp(1, 2.0) + 1.0)
                if (size > 10) size = 10
                str = gemino:::rando_seqs(1, size + 1)
                gemino:::add_insertion(vars, v = v, scaff = s,
                                       nucleos_ = str, new_pos_ = pos)
                ts[(s+1)] <- paste0(substr(ts[(s+1)], 1, pos), str,
                ...
                gemino:::add_deletion(vars, v = v, scaff = s,
