...
/*
 ...
 read nucleotides.
 */
class IlluminaQualitySampler {

public:

    std::vector<TableSampler> samplers;
    std::vector<uint8> quals;

    IlluminaQualitySampler() : sampler(), quals() {};
    IlluminaQualitySampler(const std::vector<double>& probs_,
                           const std::vector<uint8>& quals_)
        : sampler(probs_), quals(quals_), nt_conv(256, 0U) {

        for (uint32 i = 0; i < 4; i++) {
            nt_conv[table_sampler::bases[i]] = i;
        }

    }

    IlluminaQualitySampler& operator=(const IlluminaQualitySampler& other) {
        this->sampler = other.sampler;
        this->quals = other.quals;
        this->nt_conv = other.nt_conv;
        return *this;
    }
...
private:
    ...
    std::vector<uint8> nt_conv;  // converts from nucleotide char to uint32 from 0 to 3
