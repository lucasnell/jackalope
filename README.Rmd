...
the proper flags.
Regardless of operating system, you should add the following to the
...
On macOS, it's a bit more work to get things working.
...
(1\) the newest version of the `clang` compiler (`clang8` at the time of this writing)
and
(2\) the `gfortran-6.1.pkg` file.
...
Next, install `gfortran` 6.1 by opening the `gfortran-6.1.pkg` file.
Now you should be able to install `jackalope` using the `install.packages` command above.
